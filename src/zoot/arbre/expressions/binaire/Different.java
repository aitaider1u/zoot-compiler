...
public class Different extends Binaire{
    public Different(int n, Expression ef, Expression ed) {
        super(n, ef, ed);
    }
    ...
    public void verifier() {
        ...
        if (!typeExpGauche.equals("") && !typeExpDroite.equals("")){
            if(!typeExpGauche.equals(typeExpDroite)){
                System.out.println("ERREUR SEMANTIQUE : " + this.noLigne + " ligne d'erreur : Incompatible types pour Different    ('"+typeExpGauche+"' != '"+typeExpDroite+"')\n");
                AccumulateurErreurSemantiques.getInstance().ajouter("ERREUR SEMANTIQUE : " + this.noLigne + " ligne d'erreur : Incompatible types pour Different    ('"+typeExpGauche+"' != '"+typeExpDroite+"')\n");
            }
        }
    }
